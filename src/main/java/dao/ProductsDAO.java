...
public class ProductsDAO {
	...
	public ArrayList<Products> getListProducts() {
		...
		try {
			...
			while (rs.next()) {
				...
				products.setLapdat(rs.getString("lapdat"));
				// Thêm vào danh sách
				lst.add(products);
			}
			// đống kết nối
	...
	public ArrayList<Products> getProductsByCategory(String maLoai,String dientich,String maylanh,String huong) {
		...
		try {
			cs.getConnection();
			String sql = " SELECT * FROM SANPHAM WHERE id_loaisp=? and dientich=? and maylanh=? and huong=?";
			PreparedStatement cmd = cs.cn.prepareStatement(sql);
			cmd.setInt(1, Integer.parseInt(maLoai));
			cmd.setString(2, dientich);
			cmd.setString(3, maylanh);
			cmd.setString(4, huong);
	...
	public boolean addProduct(Products pr) {
		...
		try {
			...
			cmd.setString(14, pr.getDienTich());
			cmd.setString(15, pr.isMaylanh());
			cmd.setString(16, pr.getHuong());
			cmd.setString(17, pr.getLapdat());
	...
	public boolean editProduct(Products pr) {
		...
		try {
			cs.getConnection();
			String sql = "UPDATE sanpham SET tensanpham = ?, mota = ?, giagoc = ?, khuyenmai = ?, anhchinh = ?, tinhtrang = ?, idTH = ?, id_loaisp = ?,startdate=?, enddate=?, phongcach=?, diachi=?, soluong=?,maylanh=? ,huong=?,dientich=? ,lapdat=? WHERE id = ?";
			PreparedStatement cmd = cs.cn.prepareStatement(sql);
			cmd.setString(1, pr.getTenSanPham());
			cmd.setString(2, pr.getMoTa());
			cmd.setDouble(3, pr.getGiaGoc());
			...
			cmd.setString(12,pr.getDiachi() );
			cmd.setInt(13,pr.getSoluong());
			cmd.setString(14,pr.getDiachi() );
			cmd.setString(15,pr.getMaylanh() );
			cmd.setString(16,pr.getHuong() );
			cmd.setString(17,pr.getDienTich() );
			cmd.setString(18,pr.getLapdat());
			cmd.setLong(19,pr.getId());
			cmd.executeUpdate();
			cs.cn.close();
			cmd.close();
			return true;
